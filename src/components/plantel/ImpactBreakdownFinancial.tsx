...
function renderItemLabel(category: CategoryKey, item: unknown): string {
  ...
  switch (category) {
    case 'financial_transactions':
      return `${i.type === 'INCOME' ? 'Receita' : 'Despesa'} — ${formatCurrency(i.amount as string)} (${i.status})`;
    default:
...
const CATEGORIES: CategoryKey[] = ['financial_transactions'];

interface ImpactBreakdownFinancialProps {
  impact: ImpactData;
}

export function ImpactBreakdownFinancial({ impact }: ImpactBreakdownFinancialProps) {
  return (
    <>
      {CATEGORIES.map(cat => (
        <CategoryDetail
