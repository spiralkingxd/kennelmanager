...
function renderItemLabel(category: CategoryKey, item: unknown): string {
  ...
  switch (category) {
    case 'calendar_events':
      return `${i.title || '(sem título)'} — ${i.category || ''}`;
    default:
...
const CATEGORIES: CategoryKey[] = ['calendar_events'];

interface ImpactBreakdownCalendarProps {
  impact: ImpactData;
}

export function ImpactBreakdownCalendar({ impact }: ImpactBreakdownCalendarProps) {
  return (
    <>
      {CATEGORIES.map(cat => (
        <CategoryDetail
