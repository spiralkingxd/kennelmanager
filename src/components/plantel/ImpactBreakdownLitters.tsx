...
function renderItemLabel(category: CategoryKey, item: unknown): string {
  ...
  switch (category) {
    case 'litters':
      return `Data do parto: ${formatDate(i.birth_date as string)} — ${i.status || 'N/I'}`;
    case 'matings':
      return `${i.type || 'NATURAL'} — ${formatDate(i.date as string)}`;
    case 'gestations':
      return `Início: ${formatDate(i.start_date as string)}${i.is_active ? ' (ativa)' : ''}`;
    case 'heat_cycles':
      return `${formatDate(i.start_date as string)}${i.end_date ? ` → ${formatDate(i.end_date as string)}` : ''}${i.was_mated ? ' (coberta)' : ''}`;
    default:
...
const CATEGORIES: CategoryKey[] = ['litters', 'matings', 'gestations', 'heat_cycles'];

interface ImpactBreakdownLittersProps {
  impact: ImpactData;
}

export function ImpactBreakdownLitters({ impact }: ImpactBreakdownLittersProps) {
  return (
    <>
      {CATEGORIES.map(cat => (
        <CategoryDetail
