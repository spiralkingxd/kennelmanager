...
function renderItemLabel(category: CategoryKey, item: unknown): string {
  ...
  switch (category) {
    case 'exams':
      return `${i.type || '(exame)'} — ${formatDate(i.date as string)}${i.result ? ` (${i.result})` : ''}`;
    case 'consultations':
      return `${formatDate(i.date as string)} — ${i.reason || '(motivo)'}`;
    case 'weight_history':
      return `${i.weight ? `${i.weight} kg` : ''} — ${formatDate(i.date as string)}`;
    case 'medications':
      return `${i.name || '(medicação)'} — ${i.status || ''}`;
    case 'documents':
      return `${i.type || 'Documento'} — ${i.name || '(sem nome)'}`;
    default:
...
const CATEGORIES: CategoryKey[] = ['exams', 'consultations', 'weight_history', 'medications', 'documents'];

interface ImpactBreakdownHealthProps {
  impact: ImpactData;
}

export function ImpactBreakdownHealth({ impact }: ImpactBreakdownHealthProps) {
  return (
    <>
      {CATEGORIES.map(cat => (
        <CategoryDetail
