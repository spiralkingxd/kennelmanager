...
export class PuppiesController {
  ...
  public async getAll(req: Request, res: Response, next: NextFunction) {
    try {
      const litterId = req.query.litterId as string | undefined;
      if (litterId) {
        const { data, total } = await this.service.getAll(0, 1000, litterId, isolationUserId(req));
        return res.status(200).json({ success: true, message: 'Registros listados com sucesso.', data, meta: { total, page: 1, limit: total } });
      }
      const { page, limit, skip, take } = getPaginationOptions(req.query as any);
      const { data, total } = await this.service.getAll(skip, take, undefined, isolationUserId(req));
  ...
  public async create(req: Request, res: Response, next: NextFunction) {
    try {
      const payload = createPuppiesSchema.parse(req.body);
  ...
  public async update(req: Request, res: Response, next: NextFunction) {
    try {
      const payload = updatePuppiesSchema.parse(req.body);
