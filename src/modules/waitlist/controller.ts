...
import { createWaitlistSchema, updateWaitlistSchema } from './schema';
import { AppError } from '../../shared/utils/AppError';
import { isolationUserId } from '../../shared/utils/adminHelpers';

export class WaitlistController {
  private service: WaitlistService;
  constructor() {
  ...
  public async create(req: Request, res: Response, next: NextFunction) {
    try {
      const payload = createWaitlistSchema.parse(req.body);
  ...
  public async update(req: Request, res: Response, next: NextFunction) {
    try {
      const payload = updateWaitlistSchema.parse(req.body);
  ...
  public async getMatches(req: Request, res: Response, next: NextFunction) {
    try {
      const data = await this.service.getMatches(req.params.id, isolationUserId(req));
      return res.status(200).json({ success: true, message: 'Matches encontrados.', data });
    } catch (error) { next(error); }
  }
}
